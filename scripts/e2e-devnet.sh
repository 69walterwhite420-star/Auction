...
INDEX=$(cd ../../Crown-Index && pwd)
RELAY=$(cd ../../Crown-Relay && pwd)
...
echo "== local profiles (no cursor seed — Crown-Index does not poll)"
...
splitter  = "$SPLITTER"
usdc      = "$USDC"
$(grep '^factories' "$INDEX/config/testnet.toml")
EOF
...
echo "== build crown-index + crown-relay and the game (local profiles)"
...
dfx deploy sol_rpc
if dfx canister id crown-index >/dev/null 2>&1; then
    # A live crown-index may be the old polling one (incompatible stable layout)
    # or already Crown-Index; the book is derived either way, so reinstall fresh.
    dfx canister install crown-index --mode reinstall --yes \
        --wasm "$INDEX/target/wasm32-unknown-unknown/release/crown_index.wasm" \
        --argument "(opt record { sol_rpc = opt principal \"$(dfx canister id sol_rpc)\" })"
else
...
# The paid pusher: this run submits each settlement signature through it with an
# ordinary call (no cycles), and it forwards to crown-index with cycles.
dfx deploy crown-relay --argument "(opt record {
    index = opt principal \"$(dfx canister id crown-index)\";
    allowlist = opt vec { principal \"$(dfx identity get-principal)\" } })"
dfx ledger fabricate-cycles --canister crown-relay --t 100 >/dev/null
push() { dfx canister call crown-relay submit "(\"$1\")" >/dev/null 2>&1 || true; }
echo "game=$GAME_ID"
...
DONATE_TX=$(driver donate "$SOL_RPC_URL" "$SOL_DONOR_KEYPAIR" "$RECIPIENT" "$SOL_DONATE" 2>&1 | sed -n 's/^tx: //p')
echo "donate tx: $DONATE_TX"
...
A1_TX=$(driver claim "$SOL_RPC_URL" "$SOL_DONOR_KEYPAIR" "$E_A1" 0 "$SIG_A1" "$R_A" 2>&1 | sed -n 's/^tx: //p')
A2_TX=$(driver claim "$SOL_RPC_URL" "$SOL_DONOR_KEYPAIR" "$E_A2" 0 "$SIG_A2" "$R_A" 2>&1 | sed -n 's/^tx: //p')
echo "claim(A1,0) tx: $A1_TX ; claim(A2,0) tx: $A2_TX"
...
REP=""
for _ in $(seq 1 90); do
    push "$DONATE_TX"; push "$A1_TX"; push "$A2_TX"
    REP=$(reputation "$DONOR_BLOB" "$RECIPIENT_BLOB")
