...
tree="$(cargo tree -p auction-logic --edges normal --prefix none)"
if [ "$(printf '%s\n' "$tree" | wc -l)" -ne 1 ]; then
    echo "FAIL: auction-logic must have zero dependencies, got:" >&2
    printf '%s\n' "$tree" >&2
    exit 1
fi
...
# 4. the canister moves no money and makes no HTTPS outcalls; its only
#    outside view is the SOL RPC canister call named in config.
...
# 6. authorization is never by caller principal.
if grep -rn 'caller' canister/src/; then
    echo "FAIL: caller-based reference in canister/src" >&2
    exit 1
fi

# 7. the mainnet profile never names Custom RPC sources (core-spec §8).
#    Comments are stripped first: the rule is about values.
if sed 's/#.*//' config/mainnet.toml | grep -n 'Custom'; then
    echo "FAIL: Custom RPC source in config/mainnet.toml" >&2
    exit 1
fi
