//! Certified registry state (docs/game-spec.md §7): a hash tree over the
//! stored auction, lot and entry records under the label `auction`. Every
//! `get_auction`/`get_lot` answer carries the IC certificate plus a witness,
//! so the server proves one record's state — the reveal of a text, the
//! attribution of a return — against the NNS root key without mirroring the
//! whole canister.
...
pub const LABEL: &[u8] = b"auction";

thread_local! {
    /// storage key → sha256(stored record bytes). Rebuilt from stable memory
    /// on upgrade; certified data always mirrors its root.
    static TREE: RefCell<RbTree<Vec<u8>, Hash>> = const { RefCell::new(RbTree::new()) };
}

/// Records the current bytes of a record and re-certifies the root.
...
/// CBOR witness for one storage key, wrapped under the label — the proof
/// path from the certified root down to sha256(record bytes).
