//! Vote weight: the single inter-canister seam — a replicated call into
//! crown-index's book (docs/game-spec.md §10). The book is the only
//! authority on weight; there is no snapshot and no cache.
...
/// baked config value. `None` means every vote errors cleanly.
